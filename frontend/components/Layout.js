...
const Layout = ({ children }) => {
    ...
    useEffect(() => {
        ...
        async function validate() {
            try {
                ...
                console.error('Backend Server is On Sleep-mode!', error);
            }
            ...
            if ('serviceWorker' in navigator && window.location.hostname !== 'localhost') {
                navigator.serviceWorker
                    .register(process.env.NEXT_PUBLIC_HOME + '/service-worker.js', {
                        scope: '/',
                    })
                    .then((registration) => {
    ...
    const hideNavbarFooter = ['/login', '/login/otp'].includes(pathname);
    ...
    return (
        ...
        >
            <div>
                {
                    //Dynamic Navbar
                    hideNavbarFooter ? <div style={{ display: 'none' }}></div> : <Navbar />
                }
                <main>{children}</main>
                {
                    //Dynamic Footer
                    pathname.startsWith('/profile') ||
                    pathname.startsWith('/teams/68') ||
                    hideNavbarFooter ? (
