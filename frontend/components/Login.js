...
export function SignupForm({ onSwitch }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setLoading(true);
        const otp = Array.from({ length: 10 }, (_, i) => i)
            .sort(() => Math.random() - 0.5)
            .slice(0, 4)
            .join('');
        const val = {
            //Storing data in localstorage for future use in `/login/otp`...
            otp,
            fullName,
            email,
            password,
        ...
        let res = await fetch(process.env.NEXT_PUBLIC_BACKEND_URL + '/api/auth/sendEmail', {
