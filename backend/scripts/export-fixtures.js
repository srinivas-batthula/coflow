...
(async () => {
    try {
        ...
        let hackathons = await mongoose
            .model('hackpilot_hackathons', Hackathon.schema)
            .find({})
            .limit(2);
        let teams = await mongoose.model('hackpilot_teams', Team.schema).find({}).limit(2);

        fs.writeFileSync(
            path.join(__dirname, '../tests/fixtures/users.json'),
            JSON.stringify(users, null, 2)
        );
        fs.writeFileSync(
            path.join(__dirname, '../tests/fixtures/hackathons.json'),
            JSON.stringify(hackathons, null, 2)
        );
        fs.writeFileSync(
            path.join(__dirname, '../tests/fixtures/teams.json'),
