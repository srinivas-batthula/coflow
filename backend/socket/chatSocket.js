  ...
  socket.on(
    ...
    async ({ message, teamId, teamName, members_ids, userId }) => {
      ...
      if (result.success && result.data) {
        ...
        await sendPushToOfflineUsers(io, members_ids, { title: `${socket.user.fullName} sent a message in team -${teamName}`, body: `${message.slice(0, 30)}${message.length > 30 ? '...' : ''}`, id: teamId }).catch((err)=>{console.log(err)});
      }
    }
  );
