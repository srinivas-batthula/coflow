...
module.exports = (io, socket) => {
  ...
  socket.on(
    ...
    async ({ task, assigned_to, description, deadline, teamId, teamName }) => {
      ...
      if (result.success && result.data) {
        ...
        await sendPushToOfflineUsers(io, [assigned_to], { title: `New Task Alert from ${teamName}`, body: `${result.data.task.slice(0, 30)}${result.data.task.length > 30 ? '...' : ''}`, id: teamId }).catch((err)=>{console.log(err)});
      }
    }
  );
  ...
  socket.on("task_review", async ({ taskId, leaderId }) => {
    ...
    if (result.success && result.data) {
      ...
      await sendPushToOfflineUsers(io, [leaderId], { title: `Task Submitted for review by ${socket.user.fullName}`, body: `${result.data?.task.slice(0, 30)}${result.data?.task.length > 30 ? '...' : ''}`, id: result.data?.teamId }).catch((err)=>{console.log(err)});
    }
  });
  ...
  socket.on(
    ...
    async ({ taskId, teamName, assigned_to }) => {
      ...
      if (result.success && result.data) {
        ...
        // Push Notification to `assignee`...
        await sendPushToOfflineUsers(io, [assigned_to], { title: `Your Task Approved in ${teamName}`, body: `${result.data?.task.slice(0, 30)}${result.data?.task.length > 30 ? '...' : ''}`, id: result.data?.teamId }).catch((err)=>{console.log(err)});
      }
    }
  );
  ...
  socket.on("task_comment", async ({ taskId, comment, teamName, assigned_to }) => {  // Added comment for task & re-assigned to user...
    ...
    if (result.success && result.data) {
      ...
      io.to(assigned_to).emit("task_updated", result); // To member
      // Push Notification to `assignee`...
      await sendPushToOfflineUsers(io, [assigned_to], { title: `Added comment for your Task in ${teamName}`, body: `${comment.slice(0, 30)}${comment.length > 30 ? '...' : ''}`, id: result.data?.teamId }).catch((err)=>{console.log(err)});
    }
  });
};