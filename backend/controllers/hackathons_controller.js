...
const get_hackathons = async (req, res) => {
  try {
    ...
    if (result.length === 0) data = await readFallbackJson();
    else data = result;
...
const update_hackathons = async (req, res) => {
  //Requires `pass` -secret password...
  const { pass } = req.body;

  if (pass !== 'bsp_hack') {
    return res.status(401).json({
      success: false,
      details: 'Secrets Sent were Invalid/Not Matched!',
    });
  }
  try {
    res.status(201).json({
      success: true,
      details: 'Hackathons will be Fetched in 2-mins! (see logs in `render` for more details)',
    });
    ...
    console.error('Error while Scraping hackathons' + error);
