...
async function unstop(page) {
    ...
    try {
        ...
        data = await helper_Scrape(
            url + '&location-points=12.97194:77.59369',
            'Bengaluru',
            5,
            page
        );
        ...
        data = await helper_Scrape(
            url + '&location-points=17.38405:78.45636',
            'Hyderabad',
            5,
            page
        );
        ...
        console.error('Error in UnStop scraping!' + error);
    ...
    const data = await page.$$eval(
        ...
        (tiles, { limit, city }) =>
            tiles.slice(0, limit).map((tile) => {
                ...
                const id = (() => {
                    // `ID` is used in constructing URLs...
